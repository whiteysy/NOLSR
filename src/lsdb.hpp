...
{
public:
  Lsdb(ndn::Face& face, TupleState& tuplestate, ndn::KeyChain& keyChain, 
  ...
  void
  afterFetchLsa(const ndn::Data& data);
  
  void
  onContentValidationFailed(const ndn::Data& data,
                            const ndn::security::v2::ValidationError& ve);
  void
  installStateTuple(AdjLsa& alsa);

  void
  updateStateTuple(AdjLsa& alsa1, AdjLsa& alsa2);

  void
  removeStateTuple(AdjLsa& alsa);
