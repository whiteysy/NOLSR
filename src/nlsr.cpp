...
Nlsr::Nlsr(ndn::Face& face, ndn::KeyChain& keyChain, ConfParameter& confParam)
  ...
  , m_routingTable(m_scheduler, m_state, m_fib, m_lsdb, m_namePrefixTable, m_confParam)
  , m_namePrefixTable(m_fib, m_routingTable, m_routingTable.afterRoutingChange)
  , m_lsdb(m_face, m_state, m_keyChain, m_signingInfo,
  ...
  , m_helloProtocol(m_face, m_state, m_keyChain, m_signingInfo, confParam, m_routingTable, m_lsdb)
...
void
Nlsr::registerOriginRouter()
{
  for(auto it = m_adjacencyList.begin(); it != m_adjacencyList.end(); it++)
  {
    registerStrategyForCerts(it->getName());
  }
}
...
Nlsr::setDataFilter()
{
  ndn::Name helloName(m_confParam.getRouterPrefix());
  helloName.append("nlsr");
  helloName.append("INFO");

  ndn::Name lsaName("localhop/ndn/nlsr/LSA");

  NLSR_LOG_DEBUG("Setting Data filter: " << lsaName);
  NLSR_LOG_DEBUG("Setting Data filter: " << helloName);

  m_face.setDataFilter(helloName,
                       std::bind(&HelloProtocol::afterFetchHello, &m_helloProtocol, _1));
  m_face.setDataFilter(lsaName,
                       std::bind(&Lsdb::afterFetchLsa, &m_lsdb, _1));
}
...
Nlsr::initialize()
{
  ...
  m_helloProtocol.scheduleSendHello(m_confParam.getFirstHelloInterval());
